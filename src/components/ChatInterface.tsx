...
const ChatInterface = ({ avatar, preferences, onBackToHome }: ChatInterfaceProps) => {
  ...
  const handleTextToSpeech = async (messageId: string, text: string) => {
    if (ttsState.loadingId) return;

    if (ttsState.playingId === messageId && audioRef.current) {
    ...
    try {
      ...
      const response = await fetch('/api/emotional-audio', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ text: cleanText, preferences }),
      });
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const response = await fetch('/api/chat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ messages: newMessages.map(({ isUser, content }) => ({ role: isUser ? 'user' : 'assistant', content })), avatar, preferences }),
      });
  ...
  const handleSidebarVideoEnter = () => { if (sidebarVideoRef.current && !ttsState.playingId) sidebarVideoRef.current.play(); };
  const handleSidebarVideoLeave = () => { if (sidebarVideoRef.current && !ttsState.playingId) sidebarVideoRef.current.pause(); };
          ...
          <div className="flex flex-col space-y-4 max-w-4xl mx-auto">
            {messages.map((m) => (
              <div key={m.id} className={`flex items-end gap-3 ${m.isUser ? 'justify-end' : 'justify-start'}`}>
                {!m.isUser && (<video src={avatar.video} poster={avatar.image} className="w-8 h-8 rounded-full object-cover self-start" preload="metadata" muted />)}
                ...
                {!m.isUser && (
                  <Button onClick={() => handleTextToSpeech(m.id, m.content)} variant="ghost" size="icon" className="w-8 h-8 shrink-0 rounded-full text-white/50 hover:bg-white/10 hover:text-white" disabled={ttsState.loadingId !== null}>
                    {ttsState.loadingId === m.id ? <Loader2 className="w-4 h-4 animate-spin"/> : (ttsState.playingId === m.id ? <X className="w-4 h-4"/> : <Volume2 className="w-4 h-4"/>)}
                  </Button>
                )}
              </div>
            ))}
        ...
        <footer className="z-20 p-4 border-t border-white/10 bg-[#111113]/80 backdrop-blur-sm shrink-0">
          <div className="flex items-center gap-2 max-w-4xl mx-auto">
            <Input 
              ...
              onKeyPress={(e) => {
                if (e.key === 'Enter') {
                  handleSendMessage(); // Correctly call the function
                }
              }}
      ...
      <audio 
        ...
        onPlay={() => {
          if (sidebarVideoRef.current) {
            sidebarVideoRef.current.currentTime = 0;
